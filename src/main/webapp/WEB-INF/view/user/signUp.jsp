		...
		<div class="loginContainer">
			...
			<form action="/user/signUp" method="post">
				<div class="content">
					...
					<div id="ipw" class="email">
						<p>인증번호 :</p>
						<i class="material-icons">mail_outline</i> <input type="text" placeholder="이메일 인증번호를 입력하세요" name="text" id="emailCheck" class="info"><br>
						<button type="button" id="checkEmailNumber" style="margin-left: 28px;">이메일 인증</button>
					</div>
					<span class="error--messege" th:if="${valid_email}">${valid_email}</span> <br>
					<div id="ipw">
						<div class="field birth">
							...
							<div class="birthDate">
								<p>생년월일 :</p>
								<input type="number" placeholder="년(4자)" id="year" min="1900" max="2023"> <select id="month">
					...
					<span class="error--messege" th:if="${valid_tel}">${valid_tel}</span> <br>
					<c:choose>
						<c:when test="${userInfo.id != null}">
				...
				if (res['check']) {
					...
					if ('check') {
						$.ajax({
							...
							success : function(res){
								console.log(res);
								$("#checkEmailCheck").click(function() {
