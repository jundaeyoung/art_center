                     ...
                     <c:choose>
                        ...
                        <c:otherwise>
                           <form action="/admin/answer/create" method="post">
                              <textarea class="form-control summernote" rows="5" id="content" name="content">${answer.content}</textarea>
                              <input type="hidden" name="userId" value="${principal.id}"> <input type="hidden" name="questionId" value="${questionList.id}">
                              <div style="display: flex; justify-content: flex-end;">
                                 <button type="submit" class="btn" style="margin-top: 10px; border: 1px solid #ccc;">답변달기</button>
                              </div>
                           </form>
                        </c:otherwise>
                     </c:choose>
