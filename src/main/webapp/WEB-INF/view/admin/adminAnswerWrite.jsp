						...
						<c:choose>
							<c:when test="${questionList.answerStatus==1}">
								<form action="/answer/update/${questionList.id}">
									...
									<button type="submit" class="btn" style="margin-left: 1360px; margin-top: 10px; border: 1px solid #ccc;">수정하기</button>
								</form>
							</c:when>
							<c:otherwise>
								<form action="/answer/create/3/${questionList.id}">
									<textarea class="form-control summernote" rows="5" id="content" name="content">${answer.content}</textarea>
									<button type="submit" class="btn" style="margin-left: 1360px; margin-top: 10px; border: 1px solid #ccc;">답변달기</button>
								</form>
							</c:otherwise>
						</c:choose>
					</div>
